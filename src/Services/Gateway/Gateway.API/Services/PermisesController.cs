...
namespace Gateway.API.Services
{
    [Route("[controller]")]
    [ApiController]
    public class PermisesController (IConfiguration config, HttpClient client) : ControllerBase
    {
        private new readonly string Url = config["APIs:Permises"];
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PermiseDTO model, [FromHeader] string Email)
        {
            var token = await Token(Email);
            if (token == null)
                return NotFound("Token Invalid");
            ...
            client.DefaultRequestHeaders.Add("Authorization", $"Bearer {token}");
            HttpResponseMessage response = await client.PostAsJsonAsync<PermiseDTO>($"{Url}", model);
            if (response.IsSuccessStatusCode)
            {
        ...
        [HttpPut("{id:guid}")]
        public async Task<IActionResult> Edit(Guid id, [FromBody] PermiseDTO model, [FromHeader] string Email)
        {
            var token = await Token(Email);
            if (token == null)
                return NotFound("Token Invalid");
            ...
            client.DefaultRequestHeaders.Add("Authorization", $"Bearer {token}");
            HttpResponseMessage response = await client.PutAsJsonAsync<PermiseDTO>($"{Url}/{id}", model);
            if (response.IsSuccessStatusCode)
            {
                var result = response.Content.ReadAsStringAsync();
                return Content(result.Result, "application/json");
